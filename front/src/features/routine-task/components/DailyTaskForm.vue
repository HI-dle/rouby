<template>
  <div class="space-y-6">
    ...
    >
      ...
      <button
        @click="clearError"
        class="text-error-color hover:text-red-700 font-bold"
        area-label="오류 메시지 닫기"
      >
        ×
      </button>
    </div>
      ...
      <div class="flex items-center justify-between">
        ...
        >
          ...
          <div class="flex items-center w-6 justify-center">
            <!-- 저장 중 -->
            <div
              v-if="uiState.syncStatus === 'pending'"
              class="w-4 h-4 border-2 border-main-color/30 border-t-main-color rounded-full animate-spin"
            ></div>
            <!-- 저장 실패 -->
            <TriangleAlert
        ...
        <div v-else class="relative flex items-center space-x-2">
          ...
          >
            ...
            <div class="flex items-center">
              ...
              />
              <span class="text-xs text-gray-500 min-w-6">{{
                getUnitLabel()
              }}</span>
            </div>
            <button
              @click="
                handleValueChange(Math.min(uiState.displayValue + 1, 999))
              "
              :disabled="false"
